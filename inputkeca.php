...
<title>Form Input Area Kecamatan</title>
<style>
/* CSS */
h1 {
...
<div class="container">
    <h2>Form Input Area Kecamatan</h2>
    <form action="../sia/process/process_kecamatan.php" method="post">
        <div>
            <label for="id_kecamatan">ID Kecamatan:</label>
            <input type="text" id="id_kecamatan" name="id_kecamatan" required>
        </div>
        <div>
        ...
        <div>
            ...
            <select id="id_kabkota" name="id_kabkota" required>
                <?php
                // Koneksi ke database
                ...
                $sql = "SELECT id_kabkota, kabkota FROM areakabkota";
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<option value='" . $row["id_kabkota"] . "'>" . $row["id_kabkota"] . " - " . $row["kabkota"] . "</option>";
                    }
                } else {
                    echo "<option value=''>Tidak ada kabkota tersedia</option>";
                }
                // Tutup koneksi
                $conn->close();
                ?>
...
<div class="hasilinput">
    <table>
        ...
        <tbody>
            ...
            $sql = "SELECT id_kecamatan, kecamatan, id_kabkota FROM areakecamatan";
            $result = $conn->query($sql);

            // Periksa hasil query
            if ($result && $result->num_rows > 0) {
                // Tampilkan data kecamatan dalam tabel
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td class='id-kecamatan'>" . $row["id_kecamatan"] . "</td>";
                    echo "<td class='kecamatan'>" . $row["kecamatan"] . "</td>";
                    echo "<td class='id-kabkota'>" . $row["id_kabkota"] . "</td>";
                    echo "</tr>";
                }
            } else {
                echo "<tr><td colspan='3'>Tidak ada hasil input kecamatan.</td></tr>";
            }
            // Tutup koneksi
            $conn->close();
            ?>
