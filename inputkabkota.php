...
<div class="container">
    <h2>Form Input Area Kabupaten / Kota</h2>
    <form action="../sia/process/process_kabkota.php" method="post">
        <div>
            <label for="id_kabkota">ID Kabupaten / Kota:</label>
            <input type="text" id="id_kabkota" name="id_kabkota" required>
        </div>
        <div>
            <label for="kabkota">Kabupaten / Kota:</label>
            <input type="text" id="kabkota" name="kabkota" required>
        </div>
        <div>
            ...
            <select id="id_provinsi" name="id_provinsi" required>
                ...
                $sql = "SELECT id_provinsi, provinsi FROM areaprovinsi";
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<option value='" . $row["id_provinsi"] . "'>" . $row["id_provinsi"] . " - " . $row["provinsi"] . "</option>";
                    }
                } else {
...
<div class="hasilinput">
    <table>
        ...
        <tbody>
            ...
            // Query untuk mengambil data kabkota
            $sql = "SELECT id_kabkota, kabkota, id_provinsi FROM areakabkota";
            $result = $conn->query($sql);

            // Periksa hasil query
            if ($result && $result->num_rows > 0) {
                // Tampilkan data kabkota dalam tabel
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td class='id-kabkota'>" . $row["id_kabkota"] . "</td>";
                    echo "<td class='kabkota'>" . $row["kabkota"] . "</td>";
                    echo "<td class='id-provinsi'>" . $row["id_provinsi"] . "</td>";
                    echo "</tr>";
                }
            } else {
                echo "<tr><td colspan='3'>Tidak ada hasil input kabkota.</td></tr>";
            }
            // Tutup koneksi
            $conn->close();
            ?>
